...
void ChangePasswordUser()
{
    ...
    NewID = (keypad() * 100) + (keypad() * 10) + keypad();  // Collect a new ID from the keypad
    lcd_gotoxy(0, 1);  // Set cursor to the second line of the LCD
    lcd_printf("%u", NewID);  // Display the entered ID on the LCD
    delay_ms(1000);  // Delay for 1 second

    if (NewID == 126 || NewID == 128 || NewID == 130 || NewID == 132) {  // Check if the entered ID is valid
        lcd_clear();  // Clear the LCD display
        lcd_printf("Enter Old-PC");  // Display "Enter Old-PC" on the LCD
        lcd_gotoxy(0, 1);  // Set cursor to the second line of the LCD
        OldPassword = 0;  // Initialize the variable for the old password
        OldPassword = (keypad() * 100) + (keypad() * 10) + keypad();  // Collect the old password from the keypad
        storedPassword = EE_Read(NewID);  // Read the stored password from EEPROM at the given ID
        storedPassword = storedPassword + (EE_Read(NewID + 1) * 255);  // Read the second byte of the stored password
        lcd_printf("%u", OldPassword);  // Display the entered old password on the LCD
        delay_ms(1000);  // Delay for 1 second

        if (OldPassword == storedPassword) {  // Check if the entered old password matches the stored one
            // Prompt to enter the new password
            lcd_clear();
            lcd_printf("Enter New-PC");
            lcd_gotoxy(0, 1);

            NewPassword = (keypad() * 100) + (keypad() * 10) + keypad();  // Collect the new password from the keypad
            lcd_printf("%u", NewPassword);  // Display the entered new password on the LCD
            delay_ms(1000);  // Delay for 1 second
            ...
            ReenterNewPassword = (keypad() * 100) + (keypad() * 10) + keypad();  // Collect the re-entered new password
            lcd_printf("%u", ReenterNewPassword);  // Display the re-entered new password on the LCD
            delay_ms(1000);  // Delay for 1 second

            if (ReenterNewPassword == NewPassword) {  // Check if the re-entered new password matches the new password
                lcd_clear();  // Clear the LCD display
...
void ChangePasswordAdmin()
{
    ...
    lcd_gotoxy(0, 1);  // Set cursor to the second line of the LCD
    OldPassword = (keypad() * 100) + (keypad() * 10) + keypad();  // Collect the entered password
    storedPassword = EE_Read(111);  // Read the stored password for Admin from EEPROM
    storedPassword = storedPassword + (EE_Read(112) * 255);  // Read the second byte of the stored password
    lcd_printf("%u", OldPassword);  // Display the entered password on the LCD
    delay_ms(1000);  // Delay for 1 second

    if (storedPassword == OldPassword) {  // Check if the entered password matches the stored Admin password
        lcd_clear();  // Clear the LCD display
        ...
        NewID = id3 + (id2 * 10) + (id1 * 100);  // Calculate the new ID from the entered digits

        if (NewID == 111 || NewID == 126 || NewID == 128 || NewID == 130 || NewID == 132) {  // Check if the new ID is valid
            lcd_printf("%u", NewID);  // Display the new ID on the LCD
            delay_ms(1000);  // Delay for 1 second
            lcd_clear();  // Clear the LCD display
            lcd_printf("Enter-new PC: ");  // Prompt to enter the new password
            lcd_gotoxy(0, 1);  // Set cursor to the second line of the LCD
            ChangeAdminPasswords = (keypad() * 100) + (keypad() * 10) + keypad();  // Collect the new password
            lcd_printf("%u", ChangeAdminPasswords);  // Display the entered new password on the LCD
            delay_ms(1000);  // Delay for 1 second
            lcd_clear();  // Clear the LCD display
            EE_Write(NewID, ChangeAdminPasswords % 255);  // Write the lower byte of the new password to EEPROM
            EE_Write(NewID + 1, ChangeAdminPasswords / 255);  // Write the upper byte of the new password to EEPROM
            lcd_printf("Change");  // Display "Change" on the LCD
            lcd_gotoxy(0, 1);  // Set cursor to the second line of the LCD
            lcd_printf("Successfully");  // Display "Successfully" on the LCD
            delay_ms(1000);  // Delay for 1 second
        ...
        lcd_clear();  // Clear the LCD display
        delay_ms(1000);  // Delay for 1 second
        ChangePasswordAdmin();  // Call the function recursively to re-enter the password
    }
}
